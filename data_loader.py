...
class BertEmbedder:
    ...
    def get_bert_embeddings(self, examples) -> dict:
        ...
        with torch.no_grad():
            ...
            hr_last_hidden_states = move_to_cpu(hr_last_hidden_states)
        ...
        embeddings = {
            'head': hr_last_hidden_states[:, :main_args.max_word_len, :],
            'relation': hr_last_hidden_states[:, main_args.max_word_len:2*main_args.max_word_len, :],
            'tail': t_last_hidden_states[:, -main_args.max_word_len:, :]
        }
...
#         self.tokenizer = self.tokenizer_class.from_pretrained(pretrained_weights)
#         self.use_cuda = main_args.use_cuda
#         self.model_desc = self.model_class.from_pretrained(pretrained_weights)
...
#         h_token_ids = torch.tensor([f['h'] for f in features], dtype=torch.long)
#         r_token_ids = torch.tensor([f['r'] for f in features], dtype=torch.long)
#         t_token_ids = torch.tensor([f['t'] for f in features], dtype=torch.long)
...
#         with torch.no_grad():
#             h_last_hidden_states = self.model(h_token_ids)[0]  # Models outputs are now tuples
#             r_last_hidden_states = self.model(r_token_ids)[0]
#             t_last_hidden_states = self.model(t_token_ids)[0]
...
#         embeddings = {
#             'head': torch.mean(h_last_hidden_states[:, :main_args.max_word_len, :], dim=1),
#             'relation': torch.mean(r_last_hidden_states[:, main_args.max_word_len:2*main_args.max_word_len, :], dim=1),
#             'tail': torch.mean(t_last_hidden_states[:, -main_args.max_word_len:, :], dim=1)
#         }
